...
function files_path($slug)
{
    $data = [
        ...
        'social-auth-driver'    => [
            'path'              => 'backend/images/social-auth-drivers',
        ],
        'sms-provider'    => [
...
function get_full_url_host(){
    ...
    $full_url_host = $parse_base_url['host'] . '' . $parse_base_url['path'];
...
function generate_google_2fa_auth_qr() {
    ...
    $site_name = BasicSettingsProvider::get()->site_name;
    if($user->two_factor_secret) {
        $generate_text = $google2FA->getQRCodeUrl($site_name,$user->username,$user->two_factor_secret);
    }else {
        $generate_text = $google2FA->getQRCodeUrl($site_name,$user->username,$secret_key);
