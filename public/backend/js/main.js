...
$(document).ready(function(){
  $.each($(".switch-toggles-two"),function(index,item) {
    var firstSwitch = $(item).find(".switch-two").first();
    var lastSwitch = $(item).find(".switch-two").last();
    if(firstSwitch.attr('data-value') == null) {
      $(item).find(".switch-two").first().attr("data-value",true);
      $(item).find(".switch-two").last().attr("data-value",false);
    }
    if($(item).hasClass("active")) {
      $(item).find('input').val(firstSwitch.attr("data-value"));
    }else {
...
$('.switch-toggles-two .switch-two').on('click', function () {
  $(this).parents(".switch-toggles-two").toggleClass('active');
  $(this).parents(".switch-toggles-two").find("input").val($(this).attr("data-value"));
});
...
function switcherAjax(hitUrl,method = "PUT") {
    $(document).on("click",".event-ready",function(event) {
      ...
      $.post(hitUrl,data,function(response) {
        ...
        let redirect = response?.data?.redirect_url ?? false;
        if(redirect) {
...
var storedHtmlMarkup = {
  ...
  provider_credentials_field: `<div class="row align-items-end">
    <div class="col-xl-3 col-lg-3 form-group">
        <label>Title*</label>
